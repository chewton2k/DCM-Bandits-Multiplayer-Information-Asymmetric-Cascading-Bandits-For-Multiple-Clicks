...
class CascadingBandit:
    def __init__(self, num_arms, num_positions, click_probabilities):
        self.num_arms = num_arms
        self.num_positions = num_positions
        self.click_probabilities = click_probabilities
        self.w_hat = np.zeros(num_arms)  # Empirical means
        self.n = np.zeros(num_arms)      # Observation counts

    def recommend(self, selected_arms):
        click_pos = self.num_positions  # Default to no click
        for i, arm in enumerate(selected_arms):
            if random.random() < self.click_probabilities[arm]:
                click_pos = i
                break
        return click_pos
    ...
    while t < T:
        ...
        for _ in range(exploration_rounds):
            ...
            for p in range(num_players):
                # Simulate click for player p
                click_pos = num_positions  # no click by default
                for k in range(num_positions):
                    if random.random() < click_probabilities[selected_arms[k]]:
                        click_pos = k
                        break

                # Update only arms up to first click
                for k in range(min(click_pos + 1, num_positions)):
                    arm_idx = selected_arms[k]
                    reward = 1 if k == click_pos else 0
                    n[p][arm_idx] += 1
                    w_hat[p][arm_idx] = ((n[p][arm_idx] - 1) * w_hat[p][arm_idx] + reward) / n[p][arm_idx]
        ...
        for _ in range(Texploit):
            ...
            for p in range(num_players):
                Rt_p = Rt_list[p]
                click_pos = num_positions
                for k in range(num_positions):
                    if random.random() < click_probabilities[Rt_p[k]]:
                        click_pos = k
                        break

                # Update only up to the first click
                for k in range(min(click_pos + 1, num_positions)):
                    arm_idx = Rt_p[k]
                    reward = 1 if k == click_pos else 0
                    n[p][arm_idx] += 1
