...
def simulate_cascading_bandit(total_rounds, num_arms, num_positions, num_players, click_probabilities):
    ...
    for t in range(total_rounds):
        ...
        # Get only one feedback per player: first arm
        ...
        # Update only using the first arm's feedback
        for p in range(num_players):
            first_arm = recommendations[0]
            inc = clicks_all_players[p][0]  # only the first arm's feedback
            empirical_means[p][first_arm] = (
                empirical_means[p][first_arm] * observations[first_arm] + inc
            ) / (observations[first_arm] + 1)
            observations[first_arm] += 1 / num_players
